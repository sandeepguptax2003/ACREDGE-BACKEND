...
const crossDomainAuth = async (req, res, next) => {
  try {
    ...
    // For GET requests from user domain to admin routes, allow without token
    if (req.method === 'GET' && isUserDomain(origin)) {
      req.user = { role: 'user', isPublicAccess: true };
      return next();
    }
    ...
    try {
      ...
      if (req.method === 'GET' && isUserDomain(origin)) {
        // Allow GET requests from user domain even with invalid token
        req.user = { role: 'user', isPublicAccess: true };
        return next();
      }
